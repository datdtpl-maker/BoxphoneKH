...
search_dirs = [
    r"C:\Users\datdt\AppData\Roaming",
    r"C:\Users\datdt\AppData\Local"
]

print("Scanning AppData for xiaowei folders and files containing serials...")
for search_dir in search_dirs:
    for root, dirs, files in os.walk(search_dir):
        # Skip standard large directories
        if any(skip in root.lower() for skip in ['npm', 'cache', 'temp', 'microsoft', 'google', 'pip']):
            continue
        for file in files:
            filepath = os.path.join(root, file)
            # Check if path contains xiaowei or laixi
            is_relevant_path = 'xiaowei' in filepath.lower() or 'laixi' in filepath.lower()
