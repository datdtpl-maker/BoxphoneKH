...
print("Scanning LevelDB files for serial associations...")
            ...
            try:
                ...
                for serial in serials:
                    ...
                    while True:
                        ...
                        if idx == -1:
                            break
                        # Get context: 150 bytes before and after
                        start = max(0, idx - 150)
                        end = min(len(data), idx + len(serial_bytes) + 150)
                        context = data[start:end]
                        # Remove non-printable chars for cleaner output
                        context_clean = "".join([chr(c) if 32 <= c <= 126 else f"\\x{c:02x}" for c in context])
                        print(f"\nFile: {file} | Serial: {serial}")
                        print(f"Context: ... {context_clean} ...")
                        idx += len(serial_bytes) # Move past this match
